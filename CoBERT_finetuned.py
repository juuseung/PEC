...
def tokenize_conversations(data, tokenizer, max_sent_len):
    ...
    for conv in tqdm(data):
        new_conv = []
        for i, sents in enumerate(conv["context"]):
            if i==0:
                word_limit = 90
            else:
                word_limit = max_sent_len
            speaker = conv["context_speakers"][i]
            tokenized_sent = []
            for sent in sents:
        ...
        response_speaker = conv["response_speaker"]
        tokenized_response = tokenizer.tokenize(conv["response"])[:max_sent_len]
        new_data.append({
            "context": new_conv,
            "response": (response_speaker, tokenized_response)
        })
    return new_data
    ...
    for row in tqdm(data):
        ...
        if speaker in all_speakers:
            ...
            for persona in personas[:num_personas]:
                tokenized_words.extend(tokenizer.tokenize(" ".join(persona))[:22] + ["[SEP]"])
            if len(tokenized_words) > 1:
                tokenized_words.pop() # remove the last [SEP]
                new_data[speaker] = tokenized_words
...
def create_context_and_response(data):
    ...
    for conv in tqdm(data):
        ...
        for c_speaker, c_tokens in conv["context"]:
            context.extend(c_tokens + ["[SEP]"])
        if context:
            context.pop() # pop the last [SEP]
        
        # response = conv[-1][1]
        r_speaker, r_tokens = conv["response"]
        if len(context) > 0 and len(r_tokens) > 0:
            new_data.append((context, r_tokens, r_speaker))
...
def get_personas(data, num_personas):
    personas = []
    for row in tqdm(data):
        row_personas = row["personas"]
        personas.append(row_personas[:num_personas])
    return personas

def get_all_speakers(dataset):
  speakers = set()
  for split in dataset:
    for row in dataset[split]:
      speakers.update(row["context_speakers"])
      speakers.add(row["response_speaker"])
  return speakers
    
def main(config, progress):
    # save config
    # with open("./log/configs.json", "a") as f:
    with open("CoBERT_finetuned_config.json", "a") as f:
        json.dump(config, f)
        f.write("\n")
    cprint("*"*80)
    ...
    # notebook_login()
    pec_dataset = load_dataset('peixiang/pec', 'all', trust_remote_code=True) # happy # offmychest # all
    print(pec_dataset)
    ...
    # 500 100 100
    # 1000 200 200
    train_subset = pec_dataset['train'].select(range(500))
    train = train_subset

    valid_subset = pec_dataset['validation'].select(range(100))
    valid = valid_subset
    # train = load_pickle(train_path)
    # valid = load_pickle(valid_path)
    test_subset = pec_dataset['test'].select(range(100))

    if test_mode:
        # test = load_pickle(test_path)
        test = test_subset
        cprint(">>>>>>>>sample test data: ", test[0])
        # valid_path = test_path
        # valid = test
        valid_subset = pec_dataset['test'].select(range(100))
        valid = valid_subset
    ...
    if num_personas > 0:
        ...
        ## persona = get_personas(pec_dataset["train"], num_personas)

        cprint("Getting all speakers...")
        # all_speakers = get_all_speakers(pec_dataset)
        all_speakers = set(row["response_speaker"] for row in train_subset) | \
                        set(row["response_speaker"] for row in valid_subset) | \
                        set(row["response_speaker"] for row in test_subset)
        ...
        persona = tokenize_personas(chain(train_subset, valid_subset), tokenizer, all_speakers, num_personas)
    ...
    if not test_mode:
        ...
        train_dataloader = DataLoader(train_dataset, sampler=train_sampler, batch_size=batch_size, drop_last=True, num_workers=0)
    ...
    valid_dataloader = DataLoader(valid_dataset, sampler=valid_sampler, batch_size=num_candidates, num_workers=0)
    ...
    if test_mode:
        ...
        results_string = "[{0}] test loss: {1:.4f}, test acc: {2:.4f}, test recall: {3}, test MRR: {4:.4f}".format(config["result"], valid_loss, valid_acc, valid_recall, valid_MRR)
